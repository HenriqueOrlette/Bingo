...
function desenharCartela(nome, cartela) {
    var divPai = document.getElementById('cartelas');
    ...
    divCartela.appendChild(nomeJogadorElemento);
...
function gerarNumerosAleatorios() {
  ...
  function exibirNumerosAleatorios() {
    var numeros = gerarNumerosAleatorios();
    var container = document.getElementById('area_jogo');

    var numerosText = numeros.join(', ');
  
    for (var i = 0; i < numeros.length; i++) {
      var numeroElement = document.createElement('span');
      numeroElement.textContent = numeros[i];
  
      container.appendChild(numeroElement);
      
  container.textContent = numerosText;
